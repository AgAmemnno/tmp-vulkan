...
def copy_file(src,dst):
    ...
    scriptDir = os.path.dirname(os.path.realpath(__file__))
    path = os.path.join(scriptDir, src)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--libdir', type=str, help='Vulkan SDK Path.') 
    ...
    assert args.libdir ,f"Not found libdir(Vulkan SDK Path). --libdir {args.libdir}"
    ...
    loc_spec_path = os.path.normpath(args.libdir + '/vulkan/share/vulkan/registry/vk.xml')
    ...
    flushfile(f'{args.libdir}/vulkan/include/vulkan/extensions_vk.hpp')
    flushfile(f'{args.libdir}/vulkan/include/vulkan/extensions_vk.cpp')



    try:
        copy_file('extensions_vk.hpp', f'{args.libdir}/vulkan/include/vulkan/extensions_vk.hpp')
        copy_file('extensions_vk.cpp', f'{args.libdir}/vulkan/include/vulkan/extensions_vk.cpp')
    except IOError as e:
        print(e)
        sys.exit(3)
