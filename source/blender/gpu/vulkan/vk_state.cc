...
VKStateManager::VKStateManager(VKContext *_ctx) : ctx_(_ctx)
{
  ...
  /// Issue 1) In OpenGL, the dither state can change dynamically. Should this extension add a
  /// pipeline state for dither?
  /// glDisable(GL_DITHER);
  ...
  ///
  /// The Vulkan spec states: If the pipeline is being created with pre-rasterization shader state,
  /// and the wideLines feature is not enabled, and no element of the pDynamicStates member of
  /// pDynamicState is VK_DYNAMIC_STATE_LINE_WIDTH, the lineWidth member of pRasterizationState
  /// must be 1.0
                                                                    ...
                                                                    VkPipelineLayout &layout)
{
  ...
  if (current_pipeline_.inputassembly.primitiveRestartEnable == VK_TRUE) {
    bool conflict = false;
#  define EQ_TPL(name) \
    if (name == current_pipeline_.inputassembly.topology) \
      conflict = true;
...
void VKStateManager::set_provoking_vert(const eGPUProvokingVertex vert)
{
  ...
  current_pipeline_.provokingvertex.provokingVertexMode =
      (vert == GPU_VERTEX_FIRST) ? VK_PROVOKING_VERTEX_MODE_FIRST_VERTEX_EXT :
  ...
  switch (value) {
    ...
    case GPU_BLEND_ALPHA: {
      ...
      att_state.dstColorBlendFactor =
          VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;              // dst_rgb = GL_ONE_MINUS_SRC_ALPHA;
...
void VKStateManager::set_color_blend_from_fb(VKFrameBuffer *fb)
{
  ...
  VkPipelineColorBlendAttachmentState att_state_last =
      current_pipeline_.colorblend_attachment.last();
  ...
  for (auto desc : fb->get_attach_desc()) {
    ...
    /*
     * vkCreateGraphicsPipelines(): pipeline.pColorBlendState.pAttachments[2].blendEnable is
     * VK_TRUE but format VK_FORMAT_R16_UINT of the corresponding attachment description (subpass
     * 0, attachment 2) does not support VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BLEND_BIT. The Vulkan
     * spec states: If renderPass is not VK_NULL_HANDLE, and the pipeline is being created with
     * fragment output interface state, then for each color attachment in the subpass, if the
     * potential format features of the format of the corresponding attachment description do not
