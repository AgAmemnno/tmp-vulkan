...
#undef CHECK_INPUT_MODE
  ...
  Key GetMaskForTrack(
      int clip, int frame, int track, const Region *region, FloatImage *destination)
  {
    float *float_buffer;
    int width, height;
    libmv_Region libmv_region;
    ...
    Key cache_key = get_mask_for_track_callback_(user_data_,
                                                 clip,
                                                 frame,
                                                 track,
                                                 region != NULL ? &libmv_region : NULL,
...
{
  const FloatImage input(
      input_image->buffer, input_image->height, input_image->width, input_image->channels);
