...
void VKVaoCache::insert(const VKShaderInterface *interface, VKVao &vao)
{
  ...
  if (!is_dynamic_vao_count) {
    ...
    if (i < VK_GPU_VAO_STATIC_LEN) {
      ...
      dynamic_vaos.vao_ids = (VKVao *)MEM_callocN(dynamic_vaos.count * sizeof(VKVao),
                                                  "dyn vaos ids");
  ...
  if (is_dynamic_vao_count) {
    ...
    if (i == dynamic_vaos.count) {
      ...
      dynamic_vaos.vao_ids = (VKVao *)MEM_recallocN(dynamic_vaos.vao_ids,
                                                    sizeof(VKVao) * dynamic_vaos.count);
...
void VKBatch::draw(int v_first, int v_count, int i_first, int i_count)
{
  ...
  */
  ...
  /*Here, setting prim_type without fail means that there is a premise that the topology type will
