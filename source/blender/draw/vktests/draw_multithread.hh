...
#define MEASURE_TIME(NAME) \
  { \
    ...
    std::cout << "    ############PERF##########    elapsed  " << elapsed_time \
              << "  s.    ##############  " << NAME << std::endl; \
  }
...
#endif
{
...

  {
    ...
    threading::parallel_for(IndexRange(_INDEX_RANGE), 2, [&](const IndexRange range) {
      ...
      for (const int i : range) {
        ...
        // std::cout << "thread " << tid <<   "     atomap index " << val.idx << "     counter " <<
        // val.value << std::endl;
      }
    });
  ...
  for (int i = 0; i < 16; i++) {
    auto item = atomap[i];
    std::cout << "                                            Thread  " << item.idx
              << "      val=   " << item.value << std::endl;
    counter += item.value;
  }
...
#endif
{
  ...
  for (auto &task : tasks) {
    count.add_or_modify(
        task.tid, [&](int *item) { *item = 1; }, [&](int *item) { (*item)++; });
  };
