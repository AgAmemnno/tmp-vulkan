...
{
  return (libmv_AutoTrack *)LIBMV_OBJECT_NEW(AutoTrack, (FrameAccessor *)frame_accessor);
}
                               ...
                               const libmv_AutoTrackOptions *options)
{
  ...
  libmv_configureTrackRegionOptions(options->track_region, &autotrack->options.track_region);
                          ...
                          libmv_TrackRegionResult *libmv_result)
{
  ...
  bool ok = (((AutoTrack *)libmv_autotrack)->TrackMarker(&tracked_marker, &result, &options));
...
{
  Marker marker;
  int ok = ((AutoTrack *)libmv_autotrack)->GetMarker(clip, frame, track, &marker);
  if (ok) {
